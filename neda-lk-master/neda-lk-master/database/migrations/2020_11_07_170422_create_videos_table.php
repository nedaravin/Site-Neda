...
class CreateVideosTable extends Migration
{
    public function up()
    {
        Schema::create('videos', function (Blueprint $table) {
            ...
            $table->longText('video_link_en')->nullable();
            $table->longText('video_link_si')->nullable();
            $table->longText('video_link_ta')->nullable();

            $table->boolean('status')->default(1);

            $table->bigInteger('parent_id')->unsigned()->nullable();
            $table->foreign('parent_id')->references('id')->on('videos');
